...
filter ::
  ...
  -> List a
filter p =
  foldRight (\x y -> if (p x) then x :. y else y) Nil
...
seqOptional ::
  ...
  -> Optional (List a)
seqOptional os =
  foldRight (twiceOptional (:.)) (Full Nil) os
...
find ::
  ...
  -> Optional a
find p =
  foldRight (\x y -> if (p x) then (Full x) else y) Empty
